...
<h1 align="center">Геометрические методы сравнения пространств эмбеддингов</h1>
  ...
  <p align="center">
    Проект в рамках ВКР магистерской программы "Искуственный Интеллект"
  </p>
...
6. Запустить оценку метрик посредством результатов работы MLP:
...
8. Получение графиков по результатам оценки метрик:
...
5. Тепловая карта по семействам моделей для "одиночных" метрик:
   ...
   ```
    python -m scripts.plot_pairwise_error_heatmaps `
      ...
      --out_dir .\data\experiments\exp03_single_cifar10_11-03-2026\reports\family_corr_single_signed `
      --protocol signed `
      --corr_type spearman `
   ...
   ```
    python -m scripts.plot_pairwise_error_heatmaps `
      ...
      --single_metrics_dir .\data\experiments\exp03_single_cifar10_11-03-2026\single_metrics `
      --family_map_json .\data\experiments\model_families.json `
      --out_dir .\data\experiments\exp03_single_cifar10_11-03-2026\reports\family_corr_single_abs `
      --protocol abs `
      --corr_type spearman `
      --title "CIFAR10 abs" `
      --annotate
